...
/// Creates a new SPL Token mint (with 0 decimals, the NFT standard) and attaches
/// an on-chain Metaplex metadata account to it (name, symbol, URI).
///
/// Accounts:
...
pub fn create_token(accounts: &[AccountView], data: &[u8]) -> ProgramResult {
    ...
    log!("NFT mint created successfully");
    Ok(())
}
