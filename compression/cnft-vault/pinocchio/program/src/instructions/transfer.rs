...
/// real proofs well below this, the more so for `withdraw_two_cnfts`, where two
/// proofs share one transaction. Raise it (or switch to the heap-allocating
/// `invoke_signed_with_slice`) only if a deeper canopy-less tree ever needs it.
pub const MAX_PROOF_ACCOUNTS: usize = 24;
